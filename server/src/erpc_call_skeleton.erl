...
go(Connection,OrigFrom,M,F,A) ->  	
  Result = 
    try
      ...
      throw:Exception when is_record(Exception,exception) -> 
	{throw,fix_exception_stack(Exception,M,F,A)};
...
authenticate_module(Module) ->	
  AllowedModules = case application:get_env(erpc_srv,allowed_modules) of
		     undefined    -> [];
		     {ok,Modules} -> Modules
		   end,
